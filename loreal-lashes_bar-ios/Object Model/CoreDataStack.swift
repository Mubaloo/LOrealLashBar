...
class CoreDataStack {
    ...
    lazy var applicationDocumentsDirectory: NSURL = {
        // The directory the application uses to store the Core Data store file. This code uses a directory named "com.mubaloo.loreal_brush_bar_ios" in the application's documents Application Support directory.
        let urls = NSFileManager.defaultManager().URLsForDirectory(.DocumentDirectory, inDomains: .UserDomainMask)
        return urls[urls.count-1]
    }()
    
    lazy var persistentStoreURL: NSURL = {
         return self.applicationDocumentsDirectory.URLByAppendingPathComponent("LashesBarCoreData.sqlite")
    }()
    ...
    func purgePlaylist() {
        for brush in Lash.playlist(managedObjectContext) {
            brush.isInPlaylist = false
        }
